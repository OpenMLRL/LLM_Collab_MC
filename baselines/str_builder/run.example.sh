...
#   cp baselines/str_builder/run.example.sh baselines/str_builder/run.sh
#   vim baselines/str_builder/run.sh
#
# Then submit:
#   bash baselines/str_builder/run.sh baselines/str_builder/config.yaml
...
if [[ -z "${ACCOUNT}" || -z "${PARTITION}" ]]; then
  ...
  echo "Example:" >&2
  echo "  ACCOUNT=YOUR_ACCOUNT PARTITION=YOUR_PARTITION bash baselines/str_builder/run.sh baselines/str_builder/config.yaml" >&2
  exit 2
fi
...
cd \"${REPO_DIR}\"; \
python3 -u baselines/str_builder/main.py --config \"${CONFIG_PATH}\" ${EXTRA_ARGS}'"
