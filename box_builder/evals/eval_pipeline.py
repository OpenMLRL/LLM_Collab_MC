...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--num-agents", type=int, default=None, help="Number of agents (must be 2 for pipeline).")
...
def evaluate_pipeline(cfg: Dict[str, Any], args_ns: argparse.Namespace) -> Dict[str, Any]:
    ...
    num_agents = int(args_ns.num_agents or eval_cfg.get("num_agents") or 2)
    if num_agents != 2:
        raise ValueError("Pipeline evaluation expects exactly 2 agents.")
    if num_turns < 1:
        ...
        for attempt in range(num_samples):
            ...
            if verbose:
                print(
                    ...
                    f"| reward={reward:.3f} | time={total_time:.2f}s | tokens={sum(agent_tokens_total)}"
                )
