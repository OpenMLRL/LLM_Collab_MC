...
# Example SLURM wrapper for box_builder GRPO training (CoMLRL MAGRPOTrainer with num_agents=1).
#
# Usage:
#   ACCOUNT=... PARTITION=... ./box_builder/scripts/train_box_builder_grpo_sbatch.sh
#
...
CONFIG_REL="configs/box_builder_config.yaml"
...
if [[ -z "${ACCOUNT}" || -z "${PARTITION}" ]]; then
  ...
  echo "Example:" >&2
  echo "  ACCOUNT=YOUR_ACCOUNT PARTITION=YOUR_PARTITION ./box_builder/scripts/train_box_builder_grpo_sbatch.sh" >&2
  exit 2
fi

# Output dir defaults (YAML placeholders are TODO; set real paths here)
OUTPUT_BASE_DEFAULT="/projects/bevi/${USER}/output/box_builder/[jobid]"
...
sbatch \
  ...
  --mem="${MEM}" \
  --time="${TIME}" \
  --job-name="llm_collab_mc_box_builder_grpo" \
  --wrap="${WRAP_CMD}"

echo "Submitted box_builder GRPO training job with config: ${CONFIG_PATH}"
