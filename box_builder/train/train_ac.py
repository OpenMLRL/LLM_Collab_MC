...
from LLM_Collab_MC.box_builder.utils.config import apply_overrides, load_yaml, resolve_path
...
from LLM_Collab_MC.box_builder.utils.trainer_args import get_iac_args
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train box_builder with AC (single-agent IACTrainer num_agents=1)")
    parser.add_argument(
        "--config",
        type=str,
        default=os.path.join(REPO_ROOT, "box_builder", "configs", "box_builder_ac_config.yaml"),
    ...
    ac_cfg = cfg.get("ac") or cfg.get("iac") or {}
    ...
    cfg_for_args = dict(cfg)
    cfg_for_args["iac"] = dict(ac_cfg)
    iac_args = get_iac_args(cfg_for_args, model_name=model_name)
    try:
        iac_args.num_agents = 1
    except Exception:
        pass

    formatters = _build_formatters(cfg, num_agents=1, tokenizer=tokenizer)
    ...
    reward_base = get_reward_function(cfg=cfg, num_agents=1)
    ...
    if isinstance(wandb_cfg, dict) and wandb_cfg.get("enabled", True):
        ...
        tags = wandb_cfg.get(
            "tags",
            ["ac", dataset_type, "agents_1", f"turns_{num_turns_val}"],
        )
        if not isinstance(tags, list):
            tags = ["ac", dataset_type, "agents_1", f"turns_{num_turns_val}"]
        run_name = wandb_cfg.get("run_name") or "box_builder_ac"
        wandb_config = {
    ...
    trainer_kwargs: Dict[str, Any] = {
        ...
        "model_config": {
            ...
            "critic_model_kwargs": ac_cfg.get("critic_model_kwargs", model_kwargs),
    ...
    if is_multi_turn:
        ...
        def _register_split(ds: Dataset) -> None:
            ...
            for idx in range(n):
                ...
                layers_text = format_layers_text(task, world_from=w_from, include_air=include_air_rects)
                legend = legend_lines(task.palette)
                resource_limits = compute_resource_limits(task, num_agents=1) if limited_resource else {}
    ...
    if bool(output_cfg.get("save_final_model", False)):
        ...
        if save_path_cfg:
            ...
            save_path = os.path.join(os.path.abspath(iac_args.output_dir), "final_model")
