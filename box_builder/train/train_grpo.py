...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train box_builder with GRPO (single-agent MAGRPOTrainer num_agents=1)")
    parser.add_argument(
        "--config",
        type=str,
        default=os.path.join(REPO_ROOT, "box_builder", "configs", "box_builder_grpo_config.yaml"),
    ...
    grpo_cfg = cfg.get("grpo") or cfg.get("magrpo") or {}
    if not isinstance(grpo_cfg, dict):
        grpo_cfg = {}
    grpo_cfg = dict(grpo_cfg)
    grpo_cfg["num_agents"] = 1
    ...
    cfg_for_args["magrpo"] = dict(grpo_cfg)
    magrpo_args = get_trainer_args(cfg_for_args)

    formatters = _build_formatters(cfg, num_agents=1, tokenizer=tokenizer)
    reward_func = get_reward_function(cfg=cfg, num_agents=1)
    ...
    if isinstance(wandb_cfg, dict) and wandb_cfg.get("enabled", True):
        ...
        tags = wandb_cfg.get(
            "tags",
            ["grpo", dataset_type, "agents_1", f"turns_{num_turns_val}"],
        )
        if not isinstance(tags, list):
            tags = ["grpo", dataset_type, "agents_1", f"turns_{num_turns_val}"]
        run_name = wandb_cfg.get("run_name") or "box_builder_grpo"
        wandb_config = {
