...
from LLM_Collab_MC.box_builder.utils.trainer_args import get_maac_args
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train box_builder with MAAC (CoMLRL MAACTrainer)")
    parser.add_argument(
        "--config",
        type=str,
        default=os.path.join(REPO_ROOT, "box_builder", "configs", "box_builder_maac_config.yaml"),
    ...
    if isinstance(wandb_cfg, dict) and wandb_cfg.get("enabled", True):
        ...
        run_name = wandb_cfg.get("run_name") or "box_builder_maac"
        wandb_config = {
