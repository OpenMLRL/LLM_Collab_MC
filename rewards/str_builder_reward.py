...
def get_reward_function(*, cfg: Dict[str, Any], num_agents: int) -> Callable[..., List[float]]:
    ...
    Reward = score_shape_overlap + score_components + score_s3 (range ~[-2, 3]).
    """
    ...
    def _reward_from_metrics(metrics: Mapping[str, Any]) -> float:
        try:
            return (
                float(metrics.get("score_shape_overlap", 0.0))
                + float(metrics.get("score_components", 0.0))
                + float(metrics.get("score_s3", 0.0))
            )
        except Exception:
            return 0.0
    ...
    def _maybe_debug_print(
        *,
        ...
        prefix = (
            ...
            f"s3={float(metrics.get('score_s3', 0.0)):.3f}"
        )
