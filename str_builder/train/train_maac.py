...
from LLM_Collab_MC.str_builder.utils.config import apply_overrides, load_yaml, resolve_path
from LLM_Collab_MC.str_builder.utils.patches import apply_default_patches
from LLM_Collab_MC.str_builder.utils.prompting import apply_graph_setting, apply_prompt_defaults
from LLM_Collab_MC.str_builder.utils.str_builder import load_tasks_from_csv
from LLM_Collab_MC.str_builder.utils.trainer_args import get_maac_args
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train str_builder with MAAC (CoMLRL MAACTrainer)")
    parser.add_argument(
        "--config",
        type=str,
        default=os.path.join(REPO_ROOT, "str_builder", "configs", "str_builder_maac_config.yaml"),
    ...
    def _register_prompt(prompt: str, item: Mapping[str, Any], turn_idx: int) -> None:
        ...
        if not key:
            return
        mapped = dict(item)
        mapped["_str_builder_turn"] = int(turn_idx)
    ...
    import LLM_Collab_MC.str_builder.external as external_mod  # type: ignore
