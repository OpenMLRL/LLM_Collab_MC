...
# Example SLURM wrapper for str_rainbow GRPO training (CoMLRL MAGRPOTrainer with num_agents=1).
#
# Usage:
#   ACCOUNT=... PARTITION=... ./str_rainbow/scripts/train_str_rainbow_grpo_sbatch.sh
#
...
if [[ -z "${ACCOUNT}" || -z "${PARTITION}" ]]; then
  ...
  echo "Example:" >&2
  echo "  ACCOUNT=YOUR_ACCOUNT PARTITION=YOUR_PARTITION ./str_rainbow/scripts/train_str_rainbow_grpo_sbatch.sh" >&2
  exit 2
fi
...
sbatch \
  ...
  --mem="${MEM}" \
  --time="${TIME}" \
  --job-name="llm_collab_mc_str_rainbow_grpo" \
  --wrap="${WRAP_CMD}"

echo "Submitted str_rainbow GRPO training job with config: ${CONFIG_PATH}"
