...
from LLM_Collab_MC.str_rainbow.utils.trainer_args import get_iac_args
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train str_rainbow with AC (single-agent IACTrainer num_agents=1)")
    parser.add_argument(
        "--config",
        type=str,
        default=os.path.join(REPO_ROOT, "str_rainbow", "configs", "str_rainbow_ac_config.yaml"),
    ...
    cfg_for_args = copy.deepcopy(cfg)
    cfg_for_args["iac"] = copy.deepcopy(ac_cfg)
    ...
    if isinstance(wandb_cfg, dict) and wandb_cfg.get("enabled", True):
        ...
        tags = wandb_cfg.get(
            "tags",
            ["ac", dataset_type, f"agents_1", f"turns_{num_turns_val}"],
        )
        if not isinstance(tags, list):
            tags = ["ac", dataset_type, f"agents_1", f"turns_{num_turns_val}"]
        run_name = wandb_cfg.get("run_name") or "str_rainbow_ac"
        wandb_config = {
