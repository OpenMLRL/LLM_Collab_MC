...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train str_rainbow with GRPO (single-agent MAGRPOTrainer num_agents=1)")
    parser.add_argument(
        "--config",
        type=str,
        default=os.path.join(REPO_ROOT, "str_rainbow", "configs", "str_rainbow_grpo_config.yaml"),
    ...
    cfg_for_args["magrpo"] = copy.deepcopy(grpo_cfg)
    ...
    if isinstance(wandb_cfg, dict) and wandb_cfg.get("enabled", True):
        ...
        run_name = wandb_cfg.get("run_name") or "str_rainbow_grpo"
        wandb_config = {
