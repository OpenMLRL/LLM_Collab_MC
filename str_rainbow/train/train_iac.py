...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train str_rainbow with IAC (CoMLRL IACTrainer)")
    parser.add_argument(
        "--config",
        type=str,
        default=os.path.join(REPO_ROOT, "str_rainbow", "configs", "str_rainbow_iac_config.yaml"),
    ...
    if isinstance(wandb_cfg, dict) and wandb_cfg.get("enabled", True):
        ...
        run_name = wandb_cfg.get("run_name") or "str_rainbow_iac"
        wandb_config = {
